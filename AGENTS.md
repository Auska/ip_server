...
│   ├── main.cpp               # 主程序入口
│   ├── config.h/cpp           # 配置管理
│   ├── validation.h/cpp       # IP/MAC 格式验证（getaddrinfo IPv6）
│   ├── types.h                # LookupResult + ScopedTimer RAII
│   ├── database/              # 数据库抽象层
...
│   ├── mac_database.h/cpp     # OUI 数据库（SQLite RAII + 预编译语句）
...
├── tests/                      # 测试代码（9 个测试文件 + 2 个基准测试，225 个测试）
...
| `/password/generate` | GET/POST | 密码生成（GET: 单次, POST: 批量） |
...
main()
  ...
  +-- init_logging() / set_log_level()    # 日志初始化（spdlog 默认 logger）
  ...
  +-- Application（构造函数组装组件）
        ...
        +-- IPGeoHTTPServer
              ...
              +-- APIAuth (SHA-256 哈希存储密钥 + 可信代理)
              +-- Metrics (QPS, P50/P95/P99, 缓存统计)
              +-- PasswordHandler (委托 PasswordGenerator)
              +-- PasswordGenerator (多熵源随机)
              +-- httplib::Server (线程池)
...
- 密码生成通过 `PasswordHandler` 独立类处理，与 HTTP 路由解耦
- 请求中间件管线顺序: CORS -> 认证(Bearer) -> 真实 IP 提取(仅可信代理) -> 速率限制 -> 业务处理
- 批量查询策略: <=10 个顺序执行, >10 个用 `std::async` 并行执行
- 输入验证统一在 `validation.h/cpp`，IPv6 使用 `getaddrinfo()` 进行系统级验证
- 优雅关闭: `sigaction` + 原子标志 `g_shutdown_requested` + 100ms 轮询（signal-safe write 输出）
...
| httplib | `cpp-httplib` | v0.50.1 | HTTP 服务器 |
...
- **代理信任**: 统一在 APIAuth 处理，防止 X-Forwarded-For 欺骗
- **密码生成**: 8 熵源 seed_seq 增强随机性
- **输入验证**: IP/MAC 格式预验证，IPv6 使用系统级 getaddrinfo()
...
- **测试总数**: 225 个，全部通过（`CacheEdgeTest` 定义于 `test_database.cpp`）
...
- **边界覆盖**: 包含零值、空值、并发、SQL 注入、超长输入等边界场景测试
