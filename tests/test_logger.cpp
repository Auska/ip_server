...
class LoggerTest : public ::testing::Test {
   protected:
    ...
    // Get backup file path in spdlog format: filename.1.log instead of
    // filename.log.1
...
TEST_F(LoggerTest, SizeBasedRotation) {
    ...
    for (int i = 0; i < 50; i++) {
        Logger::instance().info(
            "This is a long log message to fill up the log file quickly. Message "
            "number: "
            + std::to_string(i) + " with some extra text to ensure we reach the size limit");
    }
    Logger::instance().flush();
...
TEST_F(LoggerTest, LogRotationSequence) {
    ...
    EXPECT_TRUE(std::filesystem::exists(backup2))
        << "Backup file " << backup2 << " was not created";
}

TEST_F(LoggerTest, ConfigParsingEnableFileLogging) {
    const char* argv[] = {"ip_server", "--enable-file-logging", "true", "--log-file",
                          "/tmp/test.log"};
    int argc           = 5;
...
TEST_F(LoggerTest, ConfigParsingInvalidLogMaxBackups) {
    const char* argv[] = {"ip_server", "--enable-file-logging", "true", "--log-max-backups",
                          "invalid"};
    int argc           = 5;
...
TEST_F(LoggerTest, ConfigValidationLogMaxSizeTooSmall) {
    const char* argv[] = {"ip_server", "--enable-file-logging", "true", "--log-max-size", "0"};
    int argc           = 5;
...
TEST_F(LoggerTest, ConfigValidationLogMaxSizeTooLarge) {
    const char* argv[] = {"ip_server", "--enable-file-logging", "true", "--log-max-size", "2048"};
    int argc           = 5;
